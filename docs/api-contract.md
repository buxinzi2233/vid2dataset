...
| `open_folder` | `{ path }` | `{ opened }` | Rust 本地（`open`/`xdg-open`/`explorer`），不经 bridge。错误：结构化 `{ code, message }`，`code` ∈ `PATH_NOT_FOUND` / `NOT_A_DIRECTORY` / `PERMISSION_DENIED` / `OPEN_FAILED` |
